...
constructor(
  ...
  companion object {
     ...
     *
     * @param repoId The optional ID of the repo to update. If no ID is given, all (enabled) repos
     *   will be updated. Also triggers a clean cache job if no ID is given
     */
    @UiThread
    @JvmStatic
    @JvmOverloads
      ...
      if (autoUpdate != AutoUpdateValues.Never) {
        ...
        val constraints =
          Constraints.Builder()
            ...
            .build()
        val workRequest =
          PeriodicWorkRequestBuilder<RepoUpdateWorker>(
              repeatInterval = 4,
    ...
    fun getAutoUpdateWorkInfo(context: Context): Flow<WorkInfo?> {
      return WorkManager.getInstance(context)
        .getWorkInfosForUniqueWorkFlow(UNIQUE_WORK_NAME_REPO_AUTO_UPDATE)
        .map { it.getOrNull(0) }
    }
  }
  ...
  override suspend fun doWork(): Result {
    ...
    return try {
      ...
      if (repoId >= 0) repoUpdateManager.updateRepo(repoId) else repoUpdateManager.updateRepos()
