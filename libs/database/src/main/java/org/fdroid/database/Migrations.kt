  ...
  private fun SupportSQLiteDatabase.updateRepoWeight(repo: Repo, newWeight: Int) {
    val rowsAffected =
      update(
        ...
        values = ContentValues(1).apply { put("weight", newWeight) },
        whereClause = "repoId = ?",
...
/**
 * Removes all repos without a certificate as those are broken anyway and force us to handle repos
 * without certs.
 */
...
/**
 * The tokenizer of the FTS4 table for the app metadata was modified. This migration is needed to
 * recreate the FTS table to respect the new tokenizer.
 */
...
/**
 * Somebody changed the initial IndexV2 definition of MirrorV2.location to MirrorV2.countryCode in
 * fdroidserver and doesn't want to undo this rename. So now we need to handle this in the client to
 * be in line with the index format produced.
 */
@RenameColumn(tableName = Mirror.TABLE, fromColumnName = "location", toColumnName = "countryCode")
...
/**
 * The tokenizer of the FTS4 table for the app metadata was modified. This migration is needed to
 * recreate the FTS table to respect the new tokenizer and also re-create the triggers to keep the
 * FTS table in sync with the AppMetadata table.
 */
