...
constructor(
  ...
  private val httpClientEngineFactory: HttpClientEngineFactory<*> =
    getHttpClientEngineFactory(customDns),
) {
  ...
  /**
   * Only exists because KTor doesn't keep a reference to the proxy its client uses. Should only get
   * set in [getNewHttpClient].
   */
  ...
  /**
   * Same as [get], but returns all bytes. Use this only when you are sure that a response will be
   * small. Thus, this is intentionally visible internally only. Does not use [getChannel] so, it
   * gets the [NoResumeException] as in the public API.
   */
  internal suspend fun getBytes(request: DownloadRequest, skipFirstBytes: Long? = null): ByteArray {
    val outputStream = ByteArrayOutputStream()
    outputStream.use { get(request, skipFirstBytes) { bytes, _ -> it.write(bytes) } }
  ...
  private fun resetProxyIfNeeded(proxyConfig: ProxyConfig?, mirror: Mirror? = null) {
    ...
    val newProxy =
      if (mirror.isLocal() && proxyConfig != null) {
        if (currentProxy != null)
          log.debug { "Forcing mirror to null, because mirror is local: $mirror" }
        null
      } else proxyConfig
  ...
  private fun HttpRequestBuilder.addQueryParameters() {
    ...
    this@HttpManager.parameters?.forEach { (key, value) -> parameter(key, value) }
  }
}
